function resultStruct = btEngineVectorizedFcn (dataStructInput, tradingSignal, tradingCost, maxCapAllocation)
    ...
    %                                 ] ;  % close the array
    % 
    % tradingSignal = generateTradingSignalFcn(dataInput, tradingSignalParameter)
    
    % %TODO remove when finished
    % % load sythetic data
    % path = "C:\Users\kikim\OneDrive\Documents\MATLAB_Projects\BackTradeModule_Project\Experiement";
    % fileName = "synthetic_dataset.xlsx";
    % fullFileName = fullfile(path, fileName);
    % openPrice = readtimetable(fullFileName,Sheet="openPrice") ;
    % closePrice = readtimetable(fullFileName,Sheet="closePrice") ;
    % signal = readtimetable(fullFileName,Sheet="signal") ;
    % 
    % % put into dataStruct
    % dataInputStruct.openPrice = openPrice;
    % dataInputStruct.closePrice = closePrice;
